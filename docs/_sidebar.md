* [前言](/)

-----
* [Java](java/README.md)
    * [Java异常语句打印](java/printError.md)
    * [Maven打包包括本地jar包](java/maven-include-local-jar.md)
    * [日期工具类](java/date-util.md)
    * [并发编程](java/concurrent-programming.md)
    * [日期工具类](java/date-util.md)
    * [读取配置文件](java/read-properties.md)
    * [json字符串操作](java/json-operating.md)
    * [Maven打包将依赖放到指定位置](java/maven-dependence-separate.md)
    * [微信公众号开发-day1](java/wechat-day1.md)
    * [使用花生壳进行公众号本地开发调试](java/wechat-local.md)
    * [微信公众号开发模式-day2](java/wechat-day2.md)
    * [Java生成16位随机字符串](java/random-generate.md)
    * [Base64与文件的相关操作](java/base64-and-file.md)
    * [Spring Boot](java/spring-boot.md)
        * [2.1.x: 使用SpringDataJPA完成CRUD](java/spring-boot/jpa.md)
        * [2.1.x: 整合MyBatis](java/spring-boot/mybatis.md)
        * [2.1.x: 使用Druid作为项目数据源](java/spring-boot/druid.md)
        * [2.1.x: 使用Log4j2作为日志组件](java/spring-boot/log4j2.md)
        * [2.1.x: 使用Swagger2构建RESTful API](java/spring-boot/swagger2.md)
        * [2.1.x: 集成FreeMaker、Thymeleaf模板引擎](java/spring-boot/template-engine.md)
        * [Spring Boot 2.1.x 配置多数据源的三种方式](java/spring-boot/multiple-data-source.md)
        * [ConditionOnProperty注解的使用](java/spring-boot/ConditionOnProperty.md)
    * [Spring Cloud](java/spring-cloud/README.md)
        * [Spring Boot 2.1.x版本Hystrix Dashboard出现 Unable to connect to Command Metric Stream的解决办法](java/spring-cloud/hystrix-dashboard.md)
        * [Spring Boot 2.1.x版本Zipkin Server无法启动的问题](java/spring-cloud/zipkin-server.md)
* [数据结构和算法](data-structures-and-algorithms/README.md)
    * [数据结构](data-structures-and-algorithms/data-structures/README.md)
        * [稀疏数组](data-structures-and-algorithms/data-structures/稀疏数组.md)
        * [队列](data-structures-and-algorithms/data-structures/队列.md)
        * [链表](data-structures-and-algorithms/data-structures/链表.md)
        * [栈](data-structures-and-algorithms/data-structures/栈.md)
    * 算法

-----
* [记](note/README.md)
    * [Windows10删除文件时却提示文件不存在的解决方案](note/Windows10删除文件时却提示文件不存在的解决方案.md)
    * [无法加载文件 xxxx.ps1，因为在此系统中禁止运行脚本](note/2019-11-29.md)
    * [Windows下使用自带certutil工具校验文件](note/Windows下使用自带certutil工具校验文件.md)
    * [删除office多个授权](note/删除office多个授权.md)
    * [解决Linux下环境变量失效问题](note/解决Linux下环境变量失效问题.md)
    * [Nginx配置域名访问并安装SSL证书](note/nginx-with-ssl.md)
    * [自定义安装office](note/diy-install-office.md)
    * [Windows端Tomcat部署Java-Web程序中文乱码解决](note/tomcat-in-windows.md)
    * [Linux遭遇挖矿恶意程序kdevtmpfsi，解决方案](note/linux-kdevtmpfsi.md)
    * [Windows端Tomcat部署Java-Web程序中文乱码解决](note/tomcat-in-windows.md)
    * [查询没有主键的MySQL表](note/find-no-primary-key-mysql.md)
    * [Windows自动同步一个文件夹到另一目录](note/fold-synchronization-in-windows.md)

-----
* [享](share/README.md)
    * [在Windows平台搭建hugo开发平台](share/hugo-in-windows.md)
    * [抽象带师 - 亚文化转换器](share/抽象带师 - 亚文化转换器.md)
    * [be a 韭菜(转)](share/stocking.md)
    * [VPS测速脚本](share/vps-speed-test.md)
    * [Navicat Premium 15 永久激活版安装教程](share/mavicat15.md)

